...
# Path ke file Excel
excel_path = '/home/andra/Documents/Kerjaan/engine-pribadi/plugin_check_300++_results.xlsx'

# Baca Excel
df = pd.read_excel(excel_path)

# Filter hanya baris dengan status = 'OK'
filtered_df = df[df['status'] != 'Berhasil']
...
domain_ext = ['.com', '.id', '.co.id', '.ac.id', '.sch.id', '.go.id']
...
for index, row in filtered_df.iterrows():
    ...
    scraper_code = scraper_template.replace('(nama domain)', domain).replace('(nama url)', base_url)

    # Buat file scraper
    filename = f"{domain}.py"
    path = os.path.join('/home/andra/Documents/Kerjaan/engine-pribadi/coba', filename)

    with open(path, 'w') as f:
        f.write(scraper_code)

    print(f"Scraper dibuat: {path}")
